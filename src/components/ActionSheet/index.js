...
import { Metrics, Images, Colors, Styles, Global } from '@theme/';
...
class ActionSheet extends Component {
        ...
        if (cancelButtonIndex > -1 && options[cancelButtonIndex]) {
            return (
                <TouchableHighlight
                    ...
                    style={[btnStyle.wrapper, { marginTop: 6, backgroundColor: Colors.textPrimary }]}
    ...
    _renderOptions() {
        ...
        return options.map((title, index) => {
            ...
            return index === cancelButtonIndex ? null : this._createButton(title.name, fontColor, index);
        });
    }
    render() {
        const { cancelButtonIndex } = this.props;
